        ...
        if (xhr.status >= 200 && xhr.status < 400) {
            ...
            try {
                ...
                if (response.error) {
                    ...
                    let profileUrl = `profile.html?username=${encodeURIComponent(response.username)}&xp=${encodeURIComponent(response.xp)}&email=${encodeURIComponent(response.email)}&credits=${encodeURIComponent(response.credits)}&firstname=${encodeURIComponent(response.firstname)}&lastname=${encodeURIComponent(response.lastname)}&pfp_url=${encodeURIComponent(response.pfp_url)}`;
